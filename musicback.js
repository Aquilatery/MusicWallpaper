...
var backgroundColor = "#000000"; //rgb(0,0,0)
...
function SucrosePropertyListener(name, val) {
	switch (name) {
		case "backgroundColor":
			...
			//backgroundColor = hexToRgb(val.value.substring(3));
			render()
			break
		case "useDynamicColors":
			useDynamicColors = val.value;
		...
		case "primaryColor":
			...
			defaultPrimaryColor = "#" + val.value.substring(3);
			primaryColor = "#" + val.value.substring(3);
			break
		case "secondaryColor":
			//console.log(val.value);
			defaultSecondaryColor = "#" + val.value.substring(3);
			secondaryColor = "#" + val.value.substring(3);
			break
		case "lineColor":
			lineColor = "#" + val.value.substring(3);
			render()
			break
		case "displayMusicArt":
			displayMusicArt = val.value;
			img.src = val.value ? image : null;
			render()
			break
		case "minLevel":
			minLevel = val.value;
...
async function SucroseAudioData(obj) {
	Wave(obj);
	Media(obj);
}

async function Media(obj) {
	if (obj != null && obj.State) {
		if (displayTitle)
			document.getElementById("track-title").innerHTML = obj.Title;
		else
			document.getElementById("track-title").innerHTML = "Unknown";
		if (displayArtist)
			document.getElementById("track-artist").innerHTML = obj.Artist;
		else
			document.getElementById("track-artist").innerHTML = "Unknown";

		if (obj.ThumbnailString != null && image != "data:image/png;base64," + obj.ThumbnailString) {
			img.src = null;
			const img64 = !obj.ThumbnailString.startsWith("data:image/") ?
				"data:image/png;base64," + obj.ThumbnailString :
				obj.ThumbnailString;
			// img.onload = render();
			image = img64;
			if (displayMusicArt)
			...
			render();
			setDynamicColors();
		} else if (obj.ThumbnailString == null) {
			img.src = file;
		}
	} else {
		img.src = file;
...
async function Wave(obj) {
	if (obj != null && obj.State) {
		...
		var centerX = (1474 / 1920) * canvas.width,
			...
			centerRadius = {
				w: (445 / 1920) * canvas.width,
				h: (445 / 1080) * canvas.height
			}
		isLineInner = {
			l1: false,
			l2: false,
...
function render() {
	...
	line1A = {
		x: (531 / 1920) * canvas.width,
		y: 0
	};
	line1B = {
		x: (1610 / 1920) * canvas.width,
		y: canvas.height
	};
	line2A = {
		x: (1421 / 1920) * canvas.width,
		y: 0
	};
	line2B = {
		x: canvas.width,
		y: (499 / 1080) * canvas.height
	};
	line3A = {
		x: (1529 / 1920) * canvas.width,
		y: 0
	};
	line3B = {
		x: (450 / 1920) * canvas.width,
		y: canvas.height
	};
	line4A = {
		x: canvas.width,
